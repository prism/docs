...
web {
    ...
    # Allow the server to auto-start when prism does
    auto-start = true
...
Restart Prism or use `/pr web start`. If api-key is empty, you'll see an error in the console and the web server will not start.

You can also stop the web server without stopping prism using `/pr web stop`. If auto-start is enabled, it will start again on the next Prism restart.
...
```
GET /api/v1/activities?action=block-break&causePlayer=viveleroi&since=1h
