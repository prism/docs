...
When Prism loses connection to the database, activities build up in the queue.
...
By default, wal is disabled. Writing activities to disk incurs some overhead. You must choose to enable the feature based on your needs.
...
Prism will only replay the data log if the last shutdown was "clean" (not crashed).
