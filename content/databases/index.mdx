...
A database is a place prism can store, search, and retrieve all of the data it needs.

## File-based Databases: H2, Sqlite

Recommended for small servers.
...
There are a lot of great resources comparing these choices to one another so if you have a choice between databases, please look into comparisons before choosing which Prism should use.

## Relational Databases: MySQL, MariaDB, Postgres

Recommended for medium servers (unless clickhouse is available).
...
However, if you have control over the database software installed, we strongly recommend ClickHouse.

## Analytical Databases: ClickHouse

Recommended for medium to large servers.

ClickHouse is a "columnar" database, which is an optimal data storage method for Prism's use case. It is designed to handle large volumes of data efficiently and dramatically faster than traditional relational databases.

Becaused it operates on columns and not rows, it has to less to read when querying data, and doesn't need indexing and normalization that relational databases do.

We **strongly** recommend using it when possible, especially if you plan on storing tens of millions of activities or more.
...
Prism caches data while the server is running to reduce the number of network requests. See [Caches](/concepts/caches).
...
Prism writes data to your database in batches. A single writer is usually plenty because the default batch size is 2000. However, if you want extra write throughput without changing batch size, you can enable parallel writes.

Prism allows up to 4 connections to be used for writes. This is a 4x increase in write throughput.

We highly recommend using this feature with stored procedures to avoid write conflicts. Prism is designed to handle them, but it'll be more beneficial.